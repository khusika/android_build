...
function breakfast()
{
    target=$1
    local variant=$2
    XOPLAX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/xoplax/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the XOPLAX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch xoplax_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var XOS_VERSION)
        ZIPFILE=xos-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $XOPLAX_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $XOPLAX_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/xoplax/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
